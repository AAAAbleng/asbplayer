...
const generateKaraokeSubtitleHtml = (
    ...
    // 检查下一句字幕的状态 - 轮换应该基于下一句第一个字符的调整后时间
    ...
    if (nextSubtitle && nextSubtitle.karaokeSegments && nextSubtitle.karaokeSegments.length > 0) {
        // 对于卡拉OK字幕，使用第一个字符的调整后时间
        const firstSegmentTime = nextSubtitle.karaokeSegments[0].startTime + offset + advancement;
        nextSubtitleStarted = currentTime >= firstSegmentTime;
    ...
    if (hasFinished && nextSubtitleStarted) {
        ...
        // 检查是否应该消失（当下下句开始时） - 同样基于第一个字符的调整后时间
        ...
        if (nextNextSubtitle && nextNextSubtitle.karaokeSegments && nextNextSubtitle.karaokeSegments.length > 0) {
            // 对于卡拉OK字幕，使用第一个字符的调整后时间
            const nextNextFirstSegmentTime = nextNextSubtitle.karaokeSegments[0].startTime + offset + advancement;
            if (currentTime >= nextNextFirstSegmentTime) {
                shouldShow = false; // 下下句的第一个字出现时，当前句消失
            }
        } else if (nextNextSubtitle) {
            // 对于普通字幕，使用原始开始时间
            if (currentTime >= nextNextSubtitle.start) {
                shouldShow = false;
            }
        }
    }
